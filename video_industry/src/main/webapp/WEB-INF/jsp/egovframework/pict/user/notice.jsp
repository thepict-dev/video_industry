            ...
            <c:otherwise>
                <ul class="list-container notice">
                    <c:forEach var="resultList" items="${resultList}" varStatus="status">
                        <li>
                            <a href="notice_view.do?idx=${resultList.idx}" rel="noopener noreferrer">
                                <span class="content">
                ...
                <div class="pagination">
                    <c:if test="${pictVO.pageNumber ne 1}">
                        <a class="first-page page-button" href="/notice.do?search_text=${param.search_text}&pageNumber=${pictVO.pageNumber - 10 < 1 ? 1 : pictVO.pageNumber - 10}">
                    ...
                    <c:forEach var="i" begin="${pictVO.startPage}" end="${pictVO.endPage}">
                        <c:if test="${i eq pictVO.pageNumber}">
                            <a class="page-button active" href="/notice.do?search_text=${param.search_text}&pageNumber=${i}">${i}</a>
                        </c:if>
                        <c:if test="${i ne pictVO.pageNumber}">
                            <a class="page-button" href="/notice.do?search_text=${param.search_text}&pageNumber=${i}">${i}</a>
                        </c:if>
                    </c:forEach>
        
                    <c:if test="${pictVO.lastPage ne pictVO.pageNumber}">
                        <a class="last-page page-button" href="/notice.do?search_text=${param.search_text}&pageNumber=${pictVO.pageNumber + 10 > pictVO.lastPage ? pictVO.lastPage : pictVO.pageNumber + 10}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
