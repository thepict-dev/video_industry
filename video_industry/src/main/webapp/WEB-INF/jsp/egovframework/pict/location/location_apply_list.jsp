...
<html lang="ko">
	<c:import url="../main/head.jsp">
    	<c:param name="pageTitle" value="로케이션 신청리스트"/>
    </c:import>
    
    <%@include file="../main/lnb.jsp" %>
		<c:import url="../main/header.jsp">
	    	<c:param name="title" value="로케이션관리"/>
	    	<c:param name="subtitle" value="로케이션 신청리스트"/>
	    </c:import>
	            ...
	            <div class="listInner">
	                ...
	                <div class="ListWrpper">
	                    	...
	                    	<c:forEach var="resultList" items="${resultList}" varStatus="status">
		                        <li>
		                            <c:if test="${pictVO.pageNumber eq 1}">
										<p>${location_cnt - status.index}</p>					
									</c:if>
									<c:if test="${pictVO.pageNumber ne 1}">
										<p>${location_cnt - (status.index +  ((pictVO.pageNumber - 1) * 5))}</p>
									</c:if>
		                            <p>
	                    ...
	                    <div class="pagination">
	                    	<c:if test="${pictVO.pageNumber ne 1}">
								<li><a href="/location/location_apply_list.do?search_text=${param.search_text}&pageNumber=${pictVO.pageNumber - 10 < 1 ? 1 : pictVO.pageNumber - 10}"><img src="/img/admin/prev.png" alt=""></a></li>
							</c:if>	
							
							<c:forEach var="i" begin="${pictVO.startPage}" end="${pictVO.endPage}">
								<c:if test="${i eq pictVO.pageNumber}">
									<li class="active"><a href="/location/location_apply_list.do?search_text=${param.search_text}&pageNumber=${i}" >${i}</a></li>
								</c:if>
								<c:if test="${i ne pictVO.pageNumber}">
									<li><a href="/location/location_apply_list.do?search_text=${param.search_text}&pageNumber=${i}" >${i}</a></li>
								</c:if>
							</c:forEach>	
	                    
		                    <c:if test="${pictVO.lastPage ne pictVO.pageNumber}">
								<li><a href="/location/location_apply_list.do?search_text=${param.search_text}&pageNumber=${pictVO.pageNumber + 10 > pictVO.lastPage ?  pictVO.lastPage : pictVO.pageNumber + 10}"><img src="/img/admin/next.png" alt=""></a></li>
							</c:if>
	                    </div>
	                </div>
	            </div>
		...
		<script>
			function fn_change(idx, selectValue){
				...
				$("#register").attr("action", "/location/location_cng.do");
				$("#register").submit();
		    }
			function board_mod(idx){
				location.href= "/location/location_register.do?idx="+ idx;
			}
			function board_list(){
				location.href= "/location/location_list.do";
			}

			function board_delete(idx) {
				if (confirm("삭제 하시겠습니까?")) {
					$('#idx').val(idx)
					$("#register").attr("action", "/location/location_apply_delete.do");
					$("#register").submit();
				}
			}
			function search(){
				$("#search_fm").attr("action", "/location/location_apply_list.do");
				$("#search_fm").submit();
			}
		</script>
